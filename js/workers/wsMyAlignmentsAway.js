const soccerapialignaway = async()=>{
    try {
    ...
    <div class="card-header">Alineaciones equipo Visitante</div>
    <div class="card-body">
      <h5 class="card-title">${data.teams.Match[0].AwayTeam}</h5>
      <hr>
      ...
      <p class="card-text">${data.teams.Match[0].AwayLineupForward.slice(17,34)}</p>
        ...
        `
        resultadoa+=(plantilla)
        return resultadoa
    }catch (error) {console.log(error)}
}

self.addEventListener('message', async event =>{
    let formacionesaway= await soccerapialignaway();
    self.postMessage(formacionesaway);
})